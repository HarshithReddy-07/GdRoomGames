...
export default function LobbyPage() {
  ...
  async function create() {
    ...
    try {
      const game = await api.createGame(username, numDecks);
        ...
        >
          ...
          <form onSubmit={join} className="flex flex-col gap-3">
            <input
              ...
              maxLength={6}
              required
              className="bg-black/30 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-yellow-400 uppercase tracking-widest font-mono text-center transition-colors"
            />
            <button
              type="submit"
